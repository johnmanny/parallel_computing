...
void backPropLearning(vector<example> examplesArr, neuralNet * nn) {
    ...
    while (1) {
        ...
        double outputError = 1.0, oldError = 0.0, oldActivated = 0.0; 	// variables for convergence tests
        #pragma omp parallel private(i, j, k, v, learnRate, outputError, oldError, oldActivated, bestOError, bestExampleNum) shared(outputLayer, nn) num_threads(threadCount)
    {
        bestOError = 500.0;
        bestExampleNum = 0;
        outputError = 1.0;
        oldError = 0.0;
        oldActivated = 0.0; 	// variables for convergence tests
        int exampleIter = exampleIterations;
        neuralNet threadNet;
        threadNet.layerCount = nn->layerCount;
        threadNet.layers = new layer[nn->layerCount];
        for (i = 0; i < nn->layerCount; i++) {
            threadNet.layers[i].neurons = new neuron[nn->layers[i].neuronCount];
	...
	for (i = 0; i < outputLayer; i++) {
            for (k = 0; k < nn->layers[i].neuronCount; k++) {				// for each neuron in the layer
                for (j = 0; j < nn->layers[i+1].neuronCount; j++) {			// for each connection this neuron has, randomize weight
                    nn->layers[i].neurons[k].weights[j] = 10;
		}
            }
        }
	}
	...
	for (j = 0; j < TRAININGSET; j++) {
		...
		// for each neuron in this layer
                ...
                for (v = 0; v < threadNet.layers[i].neuronCount; v++) {
                    ...
                    //threadNet.layers[i].neurons[v].activate();
                }
            }
            ...
            //threadNet.layers[outputLayer].neurons[0].derive();
            ...
            for (i = outputLayer - 1; i >= 0; i--) {
                ...
                for (v = 0; v < threadNet.layers[i].neuronCount; v++) {		// for each neuron in this layer
                    neuron * curNeuron = &threadNet.layers[i].neurons[v];
                    ...
                    for (k = 0; k < threadNet.layers[i+1].neuronCount; k++) {
                        curNeuron->curWError += (curNeuron->weights[k] * threadNet.layers[i+1].neurons[k].curWError);
                    }
                    curNeuron->derivedVal = derive(&curNeuron->activatedVal);
                    //curNeuron->derive();
                    curNeuron->curWError = curNeuron->curWError * curNeuron->derivedVal;
                    //errors[omp_get_thread_num()] += curNeuron->curWError;
                }
            }
    ...
    //// Update all weights using calculated errors
            ...
            for (i = 0; i < outputLayer; i++) {
                ...
                for (v = 0; v < threadNet.layers[i].neuronCount; v++) {
                    ...
                    neuron * curNeuron = &threadNet.layers[i].neurons[v];
                    for (k = 0; k < threadNet.layers[i+1].neuronCount; k++) {
                        curNeuron->weights[k] += (learnRate * curNeuron->activatedVal * threadNet.layers[i+1].neurons[k].curWError);
                    }
                }
            }
	...
	for (i = 0; i < outputLayer; i++) {
        ...
        for (i = 0; i < threadNet.layerCount; i++) {
            delete [] threadNet.layers[i].neurons;
        }
        delete [] threadNet.layers;
        /* ------------------------
	 ...
	 */
	/*
        if ( fabs(outputError) < 0.000002) {			// outputError threshold is arbitrary
            ...
            if ((actDiff > 0.15) && (errorDiff > 0.04)) {	// difference arbitrarily unacceptable
                ...
                cout << "\tloops through example set:\t" << exampleIter << endl;
        ...
        if ((exampleIter % 100) == 0) {
            //neuronsPrint(nn);
            cout << "\n---Convergence not achieved\n\tIterations through examples:\t\t" << exampleIter << endl;
