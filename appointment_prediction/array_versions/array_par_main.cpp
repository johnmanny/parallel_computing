...
#include <omp.h>	// for parllelization
#include "array_par_nn.h"
...
void backPropLearning(example * examplesArr, neuralNet * nn) {
	...
	for (j = 0; j < TRAININGSET; j++) {
		...
		// for each neuron in this layer
                #pragma omp parallel for schedule(auto) shared(nn, i) private(v)
	 ...
	 * 	- Second, it checks whether the old activated and error values have been getting smaller, implying
         ...
         * -----------------------
	 */
        if ( fabs(outputError) < 0.0001) {			// outputError threshold is arbitrary
